...
Adding an account requires a number used for integration which corresponds to what the carrier has on their system. If the carrier is IPEC then it also requires a Sender Id.

### Rules Fees

Buy default all rules and fees are inherited from the carrier. If you wish to customise them to what you need then click the rules fees button and update all relevant fields. 

::: warning
On update the client's carrier account will inherit these rules.
:::

LINK to RULES FEES page

### Adding a prefix

Prefixes are required for connote generation. Depending on the carrier the prefix length can vary from 3 - 8 characters long. The connote numbers 
